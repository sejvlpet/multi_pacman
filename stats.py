...
class Stats:
    ...
    def getScore(self):
       return Score(self)
        ...
        #     return 0
        #
        # # metric = self.__mean(self.pillsEaten) * self.__mean(self.firsts) - self.__mean(self.pacmansEaten)
...
class Score:
    ...
    def __gt__(self, other):
        ...
        sMetric = self.__mean(sStats.pillsEaten) / self.__mean(sStats.rounds) - self.__mean(sStats.pacmansEaten) * self.__mean(sStats.rounds)
        oMetric = other.__mean(oStats.pillsEaten) / other.__mean(oStats.rounds) - other.__mean(oStats.pacmansEaten) * other.__mean(oStats.rounds)
        ...
        if self.__mean(sStats.pillsEaten) == other.__mean(oStats.pillsEaten) and self.__mean(sStats.pacmansEaten) == other.__mean(oStats.pillsEaten):
            return self.__mean(sStats.firsts) < other.__mean(oStats.firsts)

        # if self.__mean(sStats.pillsEaten) == other.__mean(oStats.pillsEaten):
            # return self.__mean(sStats.pacmansEaten) < other.__mean(oStats.pillsEaten)
